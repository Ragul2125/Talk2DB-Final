...
SNOWFLAKE_CONFIG = {
    "user": 'naveen',
    "password": 'Naveen20051004',
    "account": 'QSJHHRI-RG78448',
...
BACKUP_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), "vector_store_backups")
METADATA_FILE = os.path.join(VECTOR_STORE_DIR, "metadata.json")

# Create necessary directories
for directory in [VECTOR_STORE_DIR, BACKUP_DIR]:
    ...
    try:
        if os.path.exists(METADATA_FILE):
            with open(METADATA_FILE, 'r') as f:
                return json.load(f)
        return {"last_backup": None, "total_queries": 0}
    except Exception as e:
...
try:
    ...
    if backups:
        ...
        shutil.copytree(latest_backup, VECTOR_STORE_DIR)
        chroma_client = chromadb.PersistentClient(
            path=VECTOR_STORE_DIR,
            settings=Settings(
                anonymized_telemetry=False,
        ...
        # If no backup exists, create fresh
        if os.path.exists(VECTOR_STORE_DIR):
            shutil.rmtree(VECTOR_STORE_DIR)
...
try:
    ...
    logging.error(f"Error initializing sentence transformer: {e}")
    raise

# Create or get collection
try:
    collection = chroma_client.get_collection("sql_queries")
except:
    collection = chroma_client.create_collection("sql_queries")

def get_similar_queries(query, threshold=0.8):
    """
    Search for similar queries in the vector store using semantic similarity.
    ...
    Returns:
        tuple: (similar_query, cached_sql) if found, (None, None) if not found
    """
    try:
        # Get query embedding
        ...
        if results and results['distances'][0][0] < (1 - threshold):
            return results['documents'][0][0], results['metadatas'][0][0]['sql_query']
        return None, None
    except Exception as e:
        logging.error(f"Error searching similar queries: {e}")
        return None, None

def store_query(query, sql_query):
    """
    Store a new query and its SQL in the vector store.
    ...
    try:
        ...
        # Create backup if needed (every 100 queries)
        if metadata["total_queries"] % 100 == 0:
            backup_vector_store()
            metadata["last_backup"] = datetime.now().isoformat()
            save_metadata(metadata)
        
        logging.info(f"Successfully stored query: {query}")
    except Exception as e:
        logging.error(f"Error storing query: {e}")

def cleanup_old_queries(max_age_days=30):
    """
    Clean up old queries from the vector store.
    
    Args:
        max_age_days (int): Maximum age of queries in days, default 30
    """
    try:
        # Get all queries
        ...
        for i, metadata in enumerate(results['metadatas']):
            query_time = float(metadata['timestamp'])
        ...
        if old_ids:
            ...
            logging.info(f"Cleaned up {len(old_ids)} old queries")
            
            # Update metadata
            metadata = load_metadata()
            metadata["total_queries"] = max(0, metadata["total_queries"] - len(old_ids))
            save_metadata(metadata)
            
    except Exception as e:
        logging.error(f"Error cleaning up old queries: {e}")
...
def get_query_stats():
    ...
    try:
        ...
        timestamps = [float(m['timestamp']) for m in results['metadatas']]
        
        return {
            "total_queries": len(results['ids']),
            "oldest_query": datetime.fromtimestamp(min(timestamps)).strftime("%Y-%m-%d %H:%M:%S"),
            "newest_query": datetime.fromtimestamp(max(timestamps)).strftime("%Y-%m-%d %H:%M:%S"),
            "last_backup": metadata.get("last_backup")
        }
    except Exception as e:
...
def init_app():
    """Initialize the application and run setup tasks."""
    cleanup_old_queries()
    # Create initial backup if none exists
    metadata = load_metadata()
    if not metadata.get("last_backup"):
        backup_vector_store()
        metadata["last_backup"] = datetime.now().isoformat()
        save_metadata(metadata)
...
def generate_sql():
    ...
    # First, check if we have a similar query in our vector store
    similar_query, cached_sql = get_similar_queries(user_query)
    
    if similar_query and cached_sql:
        # If we found a similar query, use its SQL
        sql_query = cached_sql
        query_type = "query" if sql_query.lower().startswith(("select", "show", "with",)) else "message"
        print("cached_sql",cached_sql)
        save_message(conversation_id, "system", sql_query, query_type)
        return jsonify({"conversation_id": conversation_id, "sql": sql_query, "source": "cache"})

    # If no similar query found, proceed with Gemini generation
    ...
    try:
        ...
        gen_config=genai.GenerationConfig(temperature=0.25)
        ...
        # Store the new query and its SQL in the vector store
        store_query(user_query, sql_query)
        query_type = "query" if sql_query.lower().startswith(("select", "show", "with",)) else "message"
        save_message(conversation_id, "system", sql_query, query_type)
        return jsonify({"conversation_id": conversation_id, "sql": sql_query, "source": "generated"})
    except Exception as e:
...
if __name__ == "__main__":
    init_db()
    init_app()  # Call initialization before running the app
